...
$query = "SELECT authformreceived,retainerRecieved,feewaiverreceived,longformcompleteonphone,completedlongformonline,FirstName,LastName,brand,email,brandid,longformcompleteonline FROM Status WHERE uniqueid='$uniqueid' AND tenantid='$tenantid'";
...
function getToken($status, $clientID, $uniqueid, $brandid, $brand, $tenantid) {
    ...
    $urls->OnDecline = 'https://DFWMS01.initiativelegal.com/declineToSign.php?uniqueid='.$uniqueid.'&statuslevel=Decline&document=Authorization';
...
{//wrap this in conditional -- only if never completed the long form
	...
	if (isset($authformreceived)) if (isset($longformcompleteonphone))
	{
