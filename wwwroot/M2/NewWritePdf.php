...
$month = date('Y').'-'.date('m');
$week = date('Y').'-'.date('W');
if(isset($_GET['uniqueid']) && $_GET['uniqueid'] !=null && $_GET['uniqueid'] !='')
{
    $uniqueid = $_GET['uniqueid'];
}
if (!isset($_GET['uniqueid']))
{
    if(isset($_POST['uniqueid']) && $_POST['uniqueid'] !=null && $_POST['uniqueid'] !='')
    {
        $uniqueid = $_POST['uniqueid'];
    }
}
$query3 = "SELECT brand,FirstName,LastName from Status WHERE uniqueid='".$uniqueid."'";
...
else
{
    ...
    $pdf = new PDF_Code128();
    $pdf->AddPage();
    $questiontitle = $row3['LastName'].", ".$row3['FirstName']." : ".$row3['brand']." Long Questionnare questions and answers - ".$date;
    $pdf->Ln();
    $pdf->Ln();
    ...
    while($row2 = sqlsrv_fetch_array($results2))
    {
        switch ($row2['type'])
        {
            case ($row2['type'] == 'header'):
                        ...
                        $pdf->MultiCell(0,5,$row2['Description'],'','C');
                        $pdf->Ln();  
                break;
            case ($row2['type'] == 'textonly'):
                        $pdf->SetFont('Times','B',11);
                        $pdf->MultiCell(0,5,$row2['Description']);
                break;
            ...
            foreach($row as $key => $value)
            {
                if($row2['qname'] == $key)
                {
                    if ($value !='' && $value !=null && $value !=$uniqueid)
                    {
                        ...
                        if(isset($row2['sublabel']) && $row2['sublabel'] !='' && $row2['sublabel'] !=null)
                        {
                            $pdf->MultiCell(0,5,$row2['sublabel'].': '.$value);
                        }
                        else
                        {
    ...
    $filename = $row3['LastName'].", ".$row3['FirstName']." - ".$uniqueid; 
    $filename2 = $row3['LastName'].", ".$row3['FirstName']." - LongQnA - ".$uniqueid; 
    $ext = ".pdf";
    $pdf->Output("/inetpub/wwwroot/M2/Retainerstmp/$filename2$ext","F"); //pushes file to server for temp storage
    $pdf->Output("/inetpub/wwwroot/M2/$filename2$ext","F"); //pushes file to server for temp storage
