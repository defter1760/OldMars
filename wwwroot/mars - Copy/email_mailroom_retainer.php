		  ...
		  <td width="20" bgcolor="#ffffff" >&nbsp;</td>
			<td bgcolor="#ffffff"><table width="100%" border="0"  cellpadding="14">
			  <tr>
				...
				<p>Please mail out the attached retainer packet today. <br /><br />
				Mailing address:<br />
				'.$FirstName.' '.$LastName.'
				<br />
				'.$Street1.', '.$Street2.'<br />
...
#  $mail->AddReplyTo('Lawsuittest@initiativelegal.com', 'Macy\'s Lawsuit');
  ...
  $mail->Subject = 'ACTION REQUIRED: Need to mail retainer packet! - '.$LastName.', '.$FirstName.' - '.$uniqueid;
...
$query = "IF EXISTS (SELECT * from status WHERE status.uniqueid='$uniqueid') UPDATE status set retainertomailroom='Yes',retainertomailroomdate='$date' WHERE status.uniqueid='$uniqueid'";

#$query = "IF EXISTS (SELECT * from status WHERE status.uniqueid='$uniqueid' AND status.tenantid='4' AND brand='$brand' AND brandid='$brandid') UPDATE status set retainerSent='Yes',retainerSentDate='$date',retainerSentMonth='$month',retainerSentWeek='$week' WHERE status.uniqueid='$uniqueid' AND status.tenantid='4' AND brand='$brand' AND brandid='$brandid'";
