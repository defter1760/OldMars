...
echo "<tr >";
	...
	else
	{
		...
		if (isset($_POST['snailretainer'])) if (isset($_POST['emailretainer'])) if ($_POST['emailretainer'] == 'Yes')
		{
			...
			$query = "IF EXISTS (SELECT * from status WHERE uniqueid='$uniqueid') UPDATE status set authformsent='Yes',authsentviamail='Yes',authsentviamaildate='$date',authformsentdate='$date',authformsentmonth='$month',authformsentweek='$week',authtomailroom='' WHERE uniqueid='$uniqueid'";
