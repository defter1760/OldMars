...
if (isset($postvariable)) 
{
	...
	$query = "IF EXISTS (SELECT * from status WHERE status.uniqueid='$uniqueid') UPDATE status set $varstring WHERE status.uniqueid='$uniqueid'";
...
if(strstr($notelog,'\"')){
	...
	$query = "IF EXISTS (SELECT * FROM status WHERE uniqueid='$uniqueid') UPDATE status set notelog='$newlog' WHERE uniqueid='$uniqueid'";
	$results = sqlsrv_query($conn,$query);
}
