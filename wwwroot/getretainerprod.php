...
$results = sqlsrv_query($conn,$query);
	...
	$brand = $row['brand']; if (empty($brand)) unset($brand);
  		...
  		$query = "IF EXISTS (SELECT * from status WHERE uniqueid='$uniqueid' AND tenantid='$tenantid') UPDATE status set retainerSent='Docusign',retainerSentdate='$date',retainerSentmonth='$month',retaineSentweek='$week' WHERE uniqueid='$uniqueid' AND tenantid='$tenantid'";
		$results = sqlsrv_query($conn,$query);
...
function getToken($status, $clientID, $uniqueid, $brandid, $brand, $tenantid) {
    ...
    $urls->OnDecline = "https://DFWMS01.initiativelegal.com/thanksemail1.php?Mouse";
