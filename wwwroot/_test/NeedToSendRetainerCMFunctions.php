...
?>
Retainers that need to be sent:<br />

<?PHP 
    echo "<FORM NAME ='form2' METHOD ='POST' ACTION='NeedToSendRetainerCM.php'>";
...

{
	if (empty($searchnote))
	{
		...
		$query = "SELECT * FROM status where retainerSent='No' and retainerRecieved='No' AND $search $sort";
    	$results = sqlsrv_query($conn,$query);
	}
	...
	if (isset($searchnote))
	{
	$query = "SELECT * FROM status where retainerSent='No' and retainerRecieved='No' AND $searchnote $sort";
    $results = sqlsrv_query($conn,$query);
	}
}
...
if (isset($sortstring))
{
	if (empty($searchnote))
	{
		...
		$query = "SELECT * FROM status where retainerSent='No' and retainerRecieved='No' AND $search $sorstring";
    	$results = sqlsrv_query($conn,$query);
	}
	...
	if (isset($searchnote))
	{
	$query = "SELECT * FROM status where retainerSent='No' and retainerRecieved='No' AND $searchnote $sorstring";
    $results = sqlsrv_query($conn,$query);
	}
}
...
echo "<table border='1' bordercolor='#000000' style='background-color:#FFFF99' width='100%' cellpadding='2' cellspacing='0'>";
	echo "<form action='NeedToSendRetainerCM.php?brandname=$brandname&agentlname=$agentlname' method='post'>";
...
//color every other tr color x
$colorspin = $colorspin + 1;
if( $odd = $colorspin%2 )
{
    // $odd == 1; the remainder of 25/2
    echo "<tr style='background-color:#4554F7'>";
}
else
{
    // $odd == 0; nothing remains if e.g. $number is 48 instead,
    // as in 48 / 2
    echo "<tr style='background-color:#8E90A3'>";
}
...
//
  ...
  echo "<td><font size=1><a href='https://my.spicecsm.com/srv_calls/print.php?caseid=" . $row['caseid'] . "&comp=MassAction&brand=" . $row['brandid'] . "' target='_blank'>Interview</a></td>";
echo "</tr>";
if ($showcalllog == 'Yes')
{
echo "</table>";
...
if ($showcalllog == 'Yes')
{
 echo "<table border='1' bordercolor='#000000' style='background-color:#FFFF99' width='100%' cellpadding='2' cellspacing='0'>"; 
  ...
  echo "<tr>";
  echo "<td>";
    echo "Add interaction: <form action='NeedToSendRetainerCM.php?brandname=$brandname&agentlname=$agentlname' method='post'>";
