...
if (isset($brandname)) if (isset($agentlname)) $query = "SELECT * from status WHERE interviewstarted='Yes' and reachedretainerdiscussion IS NULL AND brand='$brandname' and agentlname='$agentlname' AND status.tenantid='4'";
if (isset($agentlname)) if (empty($brandname)) $query = "SELECT * from status WHERE interviewstarted='Yes' and reachedretainerdiscussion IS NULL AND agentlname='$agentlname' AND status.tenantid='4'";
if (isset($brandname)) if (empty($agentlname)) $query = "SELECT * from status WHERE interviewstarted='Yes' and reachedretainerdiscussion IS NULL AND brand='$brandname' AND status.tenantid='4'";
if (empty($brandname)) if (empty($agentlname)) $query = "SELECT * from status WHERE interviewstarted='Yes' and reachedretainerdiscussion IS NULL AND status.tenantid='4'";
#$query = "SELECT * from status WHERE status.interviewstarted='Yes' AND status.tenantid='4' AND retainerSent<>'Yes' and retainerSent<>'No'";
$results = sqlsrv_query($conn,$query);
?>

<title>Interview InComplete</title>
Showing rows where reachedretainerdiscussion Not Yes and Not No
<br><br>
<?PHP
