...
?>
Retainers to countersign::<br />

<?PHP 
    echo "<FORM NAME ='form2' METHOD ='POST' ACTION='ReceivedRetainersNotCounterSignedCM.php'>";
...

{
	if (empty($searchnote))
	{
		...
		$query = "SELECT * FROM status where retainerRecieved='Yes' AND status.tenantid='4' AND status.retainerstatus!='Received NEEDSATTN' AND status.retainercountersignsent IS NULL AND $search $sort";
    	$results = sqlsrv_query($conn,$query);
	}
	...
	if (isset($searchnote))
	{
	$query = "SELECT * FROM status where retainerRecieved='Yes' AND status.tenantid='4' AND status.retainerstatus!='Received NEEDSATTN' AND status.retainercountersignsent IS NULL AND $searchnote $sort";
    $results = sqlsrv_query($conn,$query);
	}
}
...
if ($showcalllog == 'Yes')
{
 echo "<table border='1' bordercolor='#000000' style='background-color:#FFFF99' width='100%' cellpadding='2' cellspacing='0'>"; 
  ...
  echo "<tr>";
  echo "<td>";
    echo "Add interaction: <form action='ReceivedRetainersNotCounterSignedCM.php?brandname=$brandname&agentlname=$agentlname' method='post'>";
