...
?>
Retainers with problems:<br />

<?PHP 
    echo "<FORM NAME ='form2' METHOD ='POST' ACTION='ReceivedRetainersNeedattnCM.php'>";
...

{
	if (empty($searchnote))
	{
		...
		$query = "SELECT * FROM status where status.retainerstatus='Received NEEDSATTN' AND status.retainercountersignsent IS NULL AND $search $sort";
    	$results = sqlsrv_query($conn,$query);
	}
	...
	if (isset($searchnote))
	{
	$query = "SELECT * FROM status where status.retainerstatus='Received NEEDSATTN' AND status.retainercountersignsent IS NULL AND $searchnote $sort";
    $results = sqlsrv_query($conn,$query);
	}
}
...
echo "<table border='1' bordercolor='#000000' style='background-color:#FFFF99' width='100%' cellpadding='2' cellspacing='0'>";
	echo "<form action='ReceivedRetainersNeedattnCM.php?brandname=$brandname&agentlname=$agentlname' method='post'>";
...
if ($showcalllog == 'Yes')
{
 echo "<table border='1' bordercolor='#000000' style='background-color:#FFFF99' width='100%' cellpadding='2' cellspacing='0'>"; 
  ...
  echo "<tr>";
  echo "<td>";
    echo "Add interaction: <form action='ReceivedRetainersNeedattnCM.php?brandname=$brandname&agentlname=$agentlname' method='post'>";
