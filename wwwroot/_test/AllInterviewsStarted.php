...
if (isset($brandname)) if (isset($agentlname)) $query = "SELECT * from status WHERE status.interviewstarted='Yes' AND brand='$brandname' and agentlname='$agentlname' AND status.tenantid='4'";
if (isset($agentlname)) if (empty($brandname)) $query = "SELECT * from status WHERE status.interviewstarted='Yes' AND agentlname='$agentlname' AND status.tenantid='4'";
if (isset($brandname)) if (empty($agentlname)) $query = "SELECT * from status WHERE status.interviewstarted='Yes' AND brand='$brandname' AND status.tenantid='4'";
if (empty($brandname)) if (empty($agentlname)) $query = "SELECT * from status WHERE status.interviewstarted='Yes' AND status.tenantid='4'";
#$query = "SELECT * from status WHERE status.interviewstarted='Yes' AND status.tenantid='4'";
$results = sqlsrv_query($conn,$query);
?>

<title>Interview started</title>
Showing rows where interviewstarted = Yes
<br><br>
<?PHP
