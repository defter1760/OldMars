...
formstart("retainerAcceptReject.php?uniqueid=".$uniqueid);
echo "<tr>";
echo "<td >";
textfieldmake('Unique id','','50','uniqueid');
echo "</td >";
echo "</tr>";
echo "<tr>";
				echo "<td >";
...
//		echo "<div align='right'>Emailed to Mailroom -- OK";
	...
	if ($_POST['retainerreview'] == 'Accepted')
	{
		$dateadd = date('Y').'-'.date('m').'-'.date('d');
		$timeadd = date('H').':'.date('i').':'.date('s');
		$notestring = $dateadd." @ ".$timeadd.": <strong>Accepted and Counter signed Attorney-Client Agreement</strong><br>".$notelog;
		$query = "UPDATE status set notelog='$notestring' WHERE uniqueid='$uniqueid'";
		$results = sqlsrv_query($conn,$query);
	}
}
