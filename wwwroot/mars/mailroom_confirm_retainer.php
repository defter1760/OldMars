...
formstart("mailroom_confirm_retainer.php?uniqueid=".$uniqueid);
echo "<tr >";
	if (!isset($agentlname))
	...
	else
	{
		...
		echo "<td >";
		checkboxmake('emailretainer','Yes','Mailed Attorney-Client Agreement packet','No');
		echo "</td >";
		}
		...
		{
			$eventnote = 'Mailroom has sent Attorney-Client Agreement  ('.$_POST['snailretainer'].')';
			logthisevent($eventnote,$conn,$notelog,$uniqueid);
			$query = "IF EXISTS (SELECT uniqueid from status WHERE status.uniqueid='$uniqueid' AND status.tenantid='4' AND brand='$brand' AND brandid='$brandid') UPDATE status set retainerSent='Yes',retainersentviamail='Yes',retainersentviamaildate='$date',retainerSentMonth='$month',retainerSentWeek='$week',retainertomailroom='' WHERE status.uniqueid='$uniqueid' AND status.tenantid='4' AND brand='$brand' AND brandid='$brandid'";
