...
require("upload_retainer_config.php");
...
function uploadfile($file,$uniqueid) {
	...
	if  ($errormsg=="") {
		...
		#$loc = rooturl();
		$loc = rooturl().'/upload_retainer.php?uniqueid='.$uniqueid;
...
//
	...
	system("net use X: \\10.129.3.21\MassAction DT56DsI3lPgB18jg /user:FILES\SQLSRVuser /persistent:no");
...
//ian added)-^
	...
	if ($handle = opendir($dir)) {
		while (false !== ($file = readdir($handle))) {
			if (substr($file,0,1) != "." && $file != "upload_retainer.php") {
...
<?php $sid = md5(uniqid(rand())); //unique file id ?>
 ...
 <?PHP echo 'action="upload_retainer.php?uniqueid='.$uniqueid.'">'; ?>
  ...
  <p><label for="filename"><?php echo $lang['renameto'] ?> :</label><input type="text" id="filename" name="filename" onkeyup="filetypeCheck();" <?php echo 'value="'.$LastName.', '.$FirstName.' - Attorney-Client Agreement Received - '.$dateNOW.' - '.$uniqueid.'.pdf"' ?> size="50" /></p>
...
?>
<!--<div id="footer">
  <p><a id="hiddenlink" href="?admin" onmouseover="return true;">   </a><a href="http://labs.beffa.org/w2box/"></a><a href="http://validator.w3.org/check/referer"></a><a href="http://jigsaw.w3.org/css-validator/check/referer"></a></p>
</div>-->
</div>
</body>
</html>