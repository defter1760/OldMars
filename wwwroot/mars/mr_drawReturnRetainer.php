...
$query = "select attorneyfname,attorneylname,attorneyemail,username,password from tbl_attorneyassign where attorneylname=(select agentlname from status where uniqueid='$uid');";
...
while($row = sqlsrv_fetch_array($results))
{
    ...
    $assignedattorneyemail = $row['attorneyemail']; if (empty($assignedattorneyemail)) unset($assignedattorneyemail);
	    ...
	    $query = "SELECT FirstName,LastName,uniqueid,Street1,Street2,City,State,Zipcode
            FROM status
            where uniqueid='$uid';";
...
$agentemail = $assignedattorneyemail;
$caseattorneyname = '                                                                                                                                          '.$agentname;
$caseattorneyphonenumber = '                                                                                                                                          '.$assignedattorneyphone;
$caseattorneyemail = '                                                                                                                                          macyslawsuit@initiativelegal.com';
$hellotitle = 'ATTORNEY NEWSLETTER';
...
{
    $pdf->MultiCell(0,5,'Thank you for choosing to retain Initiative Legal Group to pursue your potential wage and hour claims against Macy’s, Inc.  We have received your signed Attorney-Client Agreement and you have retained our law firm as your attorneys for your potential wage and hour claims against Macy’s.  Enclosed is a signed copy of the Agreement for your records.');
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    $pdf->MultiCell(0,5,'If you have any questions, please call me directly at '.$assignedattorneyphone.'. You may also call us at 888.792.2293, Monday through Friday.');
    $pdf->Ln();
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    ...
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    $pdf->MultiCell(0,5,'Enclosure: Attorney-Client Agreement');
}

if ($returnstatus == 'Rejected')
{
    $pdf->MultiCell(0,5,'Our law firm has received your Attorney-Client Agreement.  Unfortunately, we cannot accept the Agreement because it contains additional markings, missing initials, or a missing signature.  Please note that you have not yet retained our law firm as your attorneys for your potential wage and hour claims against Macy’s.');
    $pdf->Ln();
    $pdf->MultiCell(0,5,'Enclosed is another Attorney-Client Agreement for your review.  If you choose to retain our law firm as your attorneys for your potential wage and hour claims against Macy’s, you must complete the Agreement by initialing the bottom of the first and second page, and signing at the bottom of the second page.  Please refrain from making additional markings, striking language, or inserting additional language.');    
    $pdf->Ln();
    $pdf->MultiCell(0,5,'If you have any questions or concerns about signing the Agreement, please do not hesitate to contact me directly at '.$assignedattorneyphone.'.  You may also call us at 888.792.2293, Monday through Friday.');     
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    $pdf->MultiCell(0,5,'You may submit the Agreement to us using the enclosed self-addressed stamped envelope.  You can also fax the Agreement to us at 1.310.734.1665 or scan and email it to us at macyslawsuit@initiativelegal.com.');
    $pdf->Ln();
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    $pdf->MultiCell(0,5,'Sincerely,');
    $pdf->Image($sigFile,$sigXrej,$sigYrej,'',$sigScale);
    #$pdf->Image('VChettySig.png',10,205,'','30');
    ...
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    $pdf->MultiCell(0,5,'Enclosure: Attorney-Client Agreement and self-addressed stamped envelope');
}
...
//		$pdf->SetFont('Arial','',10);
		...
		if ($returnstatus == 'Rejected')
		{
		$filename = $row['LastName'].", ".$row['FirstName']."  - ACA Rejected letter - ".$row['uniqueid']." -  ".$assignedattorneylname;  
		}
		
		if ($returnstatus == 'Accepted')
		{
		    $filename = $row['LastName'].", ".$row['FirstName']."  - ACA Accepted letter - ".$row['uniqueid']." -  ".$assignedattorneylname; 
		}
		
		$ext = ".pdf";
if(!is_dir("\\\\SQLSRV\\c$\\inetpub\\wwwroot\\Retainerstmp\\"))
{
    mkdir("/inetpub/wwwroot/mars/Retainerstmp/");		
}
#$pdf->Output("/inetpub/wwwroot/mars/Retainerstmp/$filename$ext","F",$row['uniqueid']);
#$pdf->Output("/inetpub/wwwroot/mars/Retainerstmp/$filename$ext","I",$row['uniqueid']);
$pdf->Output("/inetpub/wwwroot/mars/$filename$ext","F",$row['uniqueid']); //pushes file to server for temp storage
$pdf->Output("/inetpub/wwwroot/mars/Retainerstmp/$filename$ext","I",$row['uniqueid']);
...
$newdiruniquid = $dir0."".$row['uniqueid'];
###
##
