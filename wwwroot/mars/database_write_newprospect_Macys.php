...
if (isset($_POST['1WhoFirstName']))
{
	...
	$whofirstname = ereg_replace("[^A-Za-z0-9]", "",  $whofirstname);

	//strip single quote
	if(strstr($whofirstname,'\''))
	{
	
	    $whofirstname = str_replace('\'','',$whofirstname);
	}
	...
	//strip underscore
	if(strstr($whofirstname,'_'))
	{
	
	    $whofirstname = str_replace('_','',$whofirstname);
	}
	//strip ampersand
	if(strstr($whofirstname,'&'))
	{
	...
	if(strstr($whofirstname,'('))
	{
		...
		$whofirstname = preg_replace('/\(|\)/','',$whofirstname);
	}
...
echo 'popup(';
echo "'http://sqlsrv.domain.initiativelegal.com/mars/client3.php?uniqueid=".$uniqueid."');";
echo 'window.close();';
echo '</script>';
?>
