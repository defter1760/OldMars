...
if (isset($feewaivequal))
{
	if ($feewaivequal == 'Yes')
	{
		...
		$query = "select notelog,feeWaiverQualifiedDate from status  WHERE uniqueid='$uniqueid'";
		$currentlog = sqlsrv_query($conn,$query);
		while($row = sqlsrv_fetch_array($currentlog))
		{
			$oldfwqualdate = $row['feeWaiverQualifiedDate'];
			if (empty($oldfwqualdate)) unset($oldfwqualdate);
			$newlog =   $date . ' @ ' . $time . ': <strong>Qualified for Fee Waiver</strong><br>' . $row['notelog'];
		}
			if (!isset($oldfwqualdate))
			{
				$query = "UPDATE status set notelog='$newlog',feeWaiverQualifiedDate='$date' WHERE uniqueid='$uniqueid'";  // feeWaiverQualifiedDate used in Case Attorney View, added by Melaina 6-29
				$results = sqlsrv_query($conn,$query);
			}
	}
}
