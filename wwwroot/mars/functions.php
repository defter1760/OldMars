...
{
//   Commented out to remove inline font styles conflicting with stylesheet - MR 6-21
//	echo "<font size='3'>";
//echo $name.":<br></font><input type='text' style='font-size:14px' size='".$width."' value='".$variable."' name='".$fieldname."'/>";
echo $name.": <input type='text' size='".$width."' value='".$variable."' name='".$fieldname."'/>";
...
//	}
	...
	{
		echo $name.":<input type='text' size='".$width."' name='".$fieldname."'>";
	}
}
...
echo $name.":<input type='text' ROWS=3 COLS=70 class='first' size='".$width."' value='".$variable."' name='".$fieldname."'/>";
}
...
echo $name.": <input type='text' size='".$width."' value='".$variable."' />";
}
...
try {
  ...
  $mail->AddCC('vchetty@initiativelegal.com','VJ Chetty');
    $mail->AddCC('png@initiativelegal.com','Pam Ng');
  $mail->AddCC('dglucksman@initiativelegal.com','Dina Glucksman');
...
function makeretainerbulk($brandname,$uniqueid,$FirstName,$LastName,$clientname,$agentname,$agentemail,$Street1,$Street2,$City,$State,$Zipcode)
{
	...
	$caseattorneyname = '                                                                                                                                          '.$agentname;
	$caseattorneyphonenumber = '                                                                                                                                          888.792.2293e';
	$caseattorneyemail = '                                                                                                                                          '.$agentemail;
	$hellotitle = 'ATTORNEY ADVERTISEMENT';
	$hello = "$clientname" . ' ("Client") ';
	$hello2 = date('F').' '.date('j').', '.date('Y');
...
$pdf->Ln(145);
	$pdf->SetFont('Times','',12);
	$pdf->MultiCell(0,5,$tenspace.'Initiative Legal Group APC','','L');
	$pdf->Ln(-1);
	$pdf->MultiCell(0,5,$tenspace.'c/o Macy\'s Lawsuit','','L');
	$pdf->Ln(-1);
	$pdf->MultiCell(0,5,$tenspace.'9663 Santa Monica Blvd., #149','','L');
	$pdf->Ln(-1);
	#$pdf->MultiCell(0,5,$tenspace.$tenspace.'2nd Floor','','L');
	#$pdf->Ln(-1);
	
	$pdf->MultiCell(0,5,$tenspace.'Beverly Hills, CA 90210','','L');
	$pdf->Ln(3);
	$pdf->SetFont('Times','B',12);
#	$pdf->MultiCell(0,5,$tenspace.'ATTORNEY ADVERTISEMENT','','L');                
	$pdf->Ln(23);
	$pdf->SetFont('Times','',18);
	$pdf->MultiCell(0,5,$yourbestphonenumber,'','L');
#        $pdf->MultiCell(0,5,$tenspace.''.$fiftyspace.''.$yourbestphonenumber,'','L');
	$pdf->Ln(2);
	$pdf->SetFont('Times','',18);
	$pdf->MultiCell(0,5,$addressline1,'','L');
	$pdf->Ln(2);
	$pdf->SetFont('Times','',18);
	$pdf->MultiCell(0,5,$addressline2,'','L');
	$pdf->Ln();
	$code = $row['uniqueid'];
	$pdf->Code128(22,177,$uniqueid,55,3);
