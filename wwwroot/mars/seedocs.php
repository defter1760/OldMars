...
if (isset($_FILES['file'])) {
	...
	uploadfile($_FILES['file'],$uniqueid);
}
...
function uploadfile($file,$uniqueid) {
	...
	if  ($errormsg=="") {
		...
		#$loc = rooturl();
		$loc = rooturl().'/seedocs.php?uniqueid='.$uniqueid;
...
if(!is_dir("$dir"))
{
	...
	if ($handle = opendir($dir)) {
		while (false !== ($file = readdir($handle))) {
			if (substr($file,0,1) != "." && $file != "seedocs.php") {
...
<meta http-equiv="Content-type" content="text/html; charset=iso-8859-1" />
 ...
 <!-- <link rel="stylesheet" type="text/css" href="<?php //echo rooturl(); ?>w2box.css" />-->
  <link rel="stylesheet" href="https://macyslawsuit.com/wp-content/themes/MacysV3/css/seeDocs.css" type="text/css" />
...
<div id="header">
 <h1><font size='2'><?php echo "<input style='width:85%; height:25px' value='".$config['w2box_title']."'>"; ?></font></h1>
</div>
...
<form method="post" <?PHP echo 'action="seedocs.php?uniqueid='.$uniqueid.'">'; ?>
...
<?php $sid = md5(uniqid(rand())); //unique file id ?>
 <form method="post" enctype="multipart/form-data"  <?PHP echo 'action="seedocs.php?uniqueid='.$uniqueid.'">'; ?>
...
<div id="filelisting">
 ...
 <table id="t1" class="sortable"  width='100%' border='0' cellspacing=1 cellpadding='1'>
  <tr >
  <th id="th1" class="lefted" width="100" height="10"><?php echo $lang['filename']; ?></th>
  <th id="th2" width="100" height="10"><?php echo $lang['date']; ?></th>
  <th id="th3" width="100" height="10"><?php echo $lang['size']; ?></th>
  <th id="th4" width="100" height="10"><?php echo $lang['type']; ?></th>
	...
	foreach ($files as $file) {
		...
		if ($config['disable_directlink'] && $file['ext']!="directory")
			...
			if ($file['ext']!="directory")
				$url = "file:\\".$config['storage_path'].'/'.rawurlencode($dlink);
			else
				$url = rawurlencode($dlink)."/";
			
			#echo '<a href="'.$url.'">';
			echo '<a href="?uniqueid='.$uniqueid.'&download='.urlencode($file['file']).'"><font size="2">';
			$maxlen=260;
		...
		echo '&nbsp;';
		if ($file['ext']!="directory") echo '<a href="?uniqueid='.$uniqueid.'&download='.urlencode($file['file']).'"></a></td>';
		echo '<td width="100"><font size="2">'.date ($lang['date_format'], $file['date']).'</font></td>';
		echo '<td>';
